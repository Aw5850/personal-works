...
var transformOperation = function transformOperation(ast, environment) {
  ...
  var head = _.first(ast),
      contructor = dictionary[head];

  if (_.isString(head) && _.isFunction(contructor)) {
...
var decideSolution = function decideSolution(x, environment) {
  ...
  if (environment === "statement") {
    if (_.isArray(x)) {
      ...
      if (_.isString(head) && !(category.statement.indexOf(head) >= 0)) {
        return {
...
var buildMembers = (function (_buildMembers) {
  function buildMembers(_x) {
    return _buildMembers.apply(this, arguments);
  }

  buildMembers.toString = function () {
    return _buildMembers.toString();
  };
...
var buildChain = (function (_buildChain) {
  function buildChain(_x2) {
    return _buildChain.apply(this, arguments);
  }

  buildChain.toString = function () {
    return _buildChain.toString();
  };
...
var dictionary = {
  ...
  "if": function _if(args, environment) {
    ...
    return {
      ...
      alternate: alternate != null ? decideSolution(alternate, "expression") : null
    };
  },
  ...
  "while": function _while(args, environment) {
  ...
  "for": function _for(args, environment) {
    ...
    var leftCode = ["var", [left]],
        bodyCode = listUtil.prepend(body, ["var", [valueName, [".", right, left]]]);
    return {
      type: "ForInStatement",
      ...
      body: {
        type: "BlockStatement",
        body: bodyCode.map(function (item) {
          return decideSolution(item, "expression");
        })
      },
      each: false
  ...
  "in": function _in(args, environment) {
    ...
    var collection = args[0],
        value = args[1],
        code = [">=", [[".", collection, ":indexOf"], value], "0"];
